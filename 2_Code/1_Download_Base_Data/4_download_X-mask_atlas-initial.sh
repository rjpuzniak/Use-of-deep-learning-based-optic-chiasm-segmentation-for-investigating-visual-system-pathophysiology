...
#Description	: This script interacts with brainlife.io CLI in order to download "X-mask_atlas-initial" of the optic chiasm, which were created by running FreeSurfer on both HCP 3T 1200 Subjects Release and CHIASM datasets.
#Args           :                                                                                           
...
for dataset in $datasets; do
	...
	for subject in $(jq -r '.[].meta.subject' all.json | sort -u); do
	    ...
	    outdir=../../1_Data/3_X-mask_atlas-initial/$dataset/$subject
	    mkdir -p $outdir
