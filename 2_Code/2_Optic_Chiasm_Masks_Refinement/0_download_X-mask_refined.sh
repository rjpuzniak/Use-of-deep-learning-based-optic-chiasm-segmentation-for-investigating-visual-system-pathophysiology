...
#Description	: This script interacts with brainlife.io CLI in order to download "X-mask refined" of the optic chiasm, which were created by refining exisiting "X-masks inital" with custom algorithm (designed for HCP data)
#Args           :                                                                                           
...
for dataset in $datasets; do
	...
	for subject in $(jq -r '.[].meta.subject' all.json | sort -u); do
	    ...
	    outdir=../../1_Data/1_T1w_Images_and_Labels/3_Optic_Chiasm_Labels_Refined/$dataset/$subject
	    mkdir -p $outdir
