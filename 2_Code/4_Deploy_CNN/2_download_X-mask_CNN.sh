...
###########################################################################################################################################################################################################################
#Script Name	: 2_download_X-mask_CNN.sh                                                               
#Description	: This script interacts with brainlife.io CLI in order to download "X-mask_CNN" of the optic chiasm, which were created by running the optmal trained CNN on the T1w images from test datasets.
#Args           :                                                                                           
...
for dataset in $datasets; do
	...
	for subject in $(jq -r '.[].meta.subject' all.json | sort -u); do
	    ...
	    outdir=../../1_Data/5_X-mask_CNN/training_30ep_00025lr/connectivity_3/threshold_1/$dataset/$subject
	    mkdir -p $outdir
